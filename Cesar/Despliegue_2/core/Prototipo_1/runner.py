# -*- coding: utf-8 -*-
"""Runner reusable del sistema experto (standalone — Prototipo_1).
...
def cargar_reglas_json(path: str | None = None) -> list[dict]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        reglas = json.load(f)
    # Convertir listas internas de condiciones a tuplas para compatibilidad
    for r in reglas:
        r["if"] = [tuple(c) if isinstance(c, list) else c for c in r.get("if", [])]
    return reglas
...
def aplicar_acciones_sobre_setpoints(acciones: list[str], setpoints: dict, limites_sp: dict) -> dict:
    ...
    for accion in acciones:
        sp_ton, sp_am, sp_ac, sp_rpm = apply_action(
            action=str(accion),
            sp_ton=sp_ton, sp_am=sp_am, sp_ac=sp_ac, sp_rpm=sp_rpm,
            ton_ll=ton_ll, ton_hl=ton_hl,
            am_ll=am_ll, am_hl=am_hl,
            ac_ll=ac_ll, ac_hl=ac_hl,
            rpm_ll=rpm_ll, rpm_hl=rpm_hl,
        )
...
def correr_prueba_general(
    ...
    if setpoints_base is None:
        raise ValueError("setpoints_base es obligatorio.")
    if limites_sp is None:
        raise ValueError("limites_sp es obligatorio.")
