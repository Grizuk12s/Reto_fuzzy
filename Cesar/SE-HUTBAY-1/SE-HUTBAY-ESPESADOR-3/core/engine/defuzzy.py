...
def step_por_accion_tabla(
    ...
    """
    # 1) Busqueda DIRECTA: si el nombre de la accion figura tal cual en la
    #    tabla de alguna familia, esa es la respuesta. Permite nombres de
    #    accion libres, definidos por cliente, sin gramatica en el string.
    accion_str = str(accion).strip()
    for fam, tabla_fam in (defuzzy_por_familia or {}).items():
        pasos = (tabla_fam or {}).get("steps_por_accion") or {}
        if accion_str in pasos:
            bx = np.array(tabla_fam["belief_axis"], dtype=float)
            sy = np.array(pasos[accion_str], dtype=float)
            b = float(max(0.0, min(1.0, float(belief))))
            return fam, float(np.interp(b, bx, sy))

    # 2) Compatibilidad: nomenclatura AUMENTAR_<FAMILIA>[_INTENSIDAD] del
    #    set original del espesador, donde la familia se deduce del nombre.
