...
# ============================================================
# Helper para cargar fuzzy de pendiente desde pendientes.json
# ============================================================
PENDIENTES_JSON_PATH = _os.path.join(_CFG_DIR, "pendientes.json")


def cargar_pendientes_json(path: str | None = None) -> dict:
    """Sin plantilla: un archivo que falta significa "no hay pendientes".

    A diferencia de los demas loaders, aqui NO se cae a nada del espesador:
    `PEND_MODELOS` es justamente lo que este archivo vino a reemplazar.
    """
    ruta = path or PENDIENTES_JSON_PATH
    try:
        with open(ruta, "r", encoding="utf-8") as _f:
            datos = _json.load(_f)
    except (OSError, ValueError):
...
def extraer_limites_fuzzy_desde_row(row: pd.Series, columnas_entrada: dict | None = None) -> dict:
    """Limites de fuzzificacion presentes en la fila.

    Tolera que falte un limite: en vivo, un tag de limite con calidad mala no
    llega a la fila y antes eso reventaba con KeyError, tumbando el tick
    entero. Lo que falta simplemente no aparece en el dict; quien fuzzifica
    decide si esa variable es evaluable (ver SEEngine._run_tick).
    """
    columnas_entrada = COLUMNAS_ENTRADA if columnas_entrada is None else columnas_entrada
    limites = {}
    for var, meta in LIMITES_FUZZY_POR_VARIABLE.items():
        bounds = {}
        for nombre, rol in meta.items():
            col = _resolver_col(rol, columnas_entrada)
            if col in row:
                bounds[nombre] = float(row[col])
        limites[var] = bounds
    return limites
...
def _evaluar_estado_fuzzy(
    ...
    limites_override: dict | None = None,
    pend_extra: dict | None = None,
    ...
    limites_override : dict | None
        Limites {var: {"lmin": x, "lmax": y}} ya resueltos por quien llama.
        El motor en vivo los arma mezclando los tags LIM leidos con el
        respaldo fijo de las variables calculadas, que no salen de la fila.
    """
    ...
    if inputs_override is not None:
        # Solo las PV que realmente llegaron: en vivo una PV con calidad mala
        # no esta en el override, y exigirla aqui tumbaba el tick completo por
        # una sola senal caida.
        inputs = {var: float(inputs_override[var])
                  for var in variables_proceso if var in inputs_override}
    else:
        inputs = extraer_inputs_desde_row(row, columnas_entrada, variables_proceso)

    limites_fuzzy = (dict(limites_override) if limites_override is not None
                     else extraer_limites_fuzzy_desde_row(row, columnas_entrada))
    ...
    # Entradas ya calculadas por quien llama: pendientes
    # (core/fuzzy/pendientes.py) y aceleraciones
    # (core/variables/aceleracion.py). El parametro se sigue llamando
    # `pend_extra` por historia; transporta las dos.
    # Entran ANTES de expandir: asi reciben sus NO-<X> como cualquier otra
    # variable y una regla puede escribir ("tendencia", "NO-INC") o
    # ("nivel_Acceleration_5s", "NO-ACELERANDO").
    if pend_extra:
        fuzzy_out.update(pend_extra)
...
def correr_prueba_general(
    ...
    fuzzy_modelos: dict | None = None,
    pend_modelos: dict | None = None,
    # Fuzzy de pendiente configurables (pendientes.json). Se evaluan con el
    # MISMO objeto que usa el motor en vivo, para que la simulacion y
    # produccion no difieran en las tendencias.
    pendientes_config: dict | None = None,
    aceleraciones_config: dict | None = None,
    variables_proceso: list[str] | None = None,
    ...
    # --- Variables calculadas: mismo evaluador que el motor en vivo ---
    # La pre-pasada `calcular_variables_df` de arriba sigue existiendo para
    # enriquecer `data_proceso` (las curvas que grafica la pagina), pero NO es
    # lo que decide: calcula sobre la columna CRUDA y con un dt fijo. Lo que
    # ve el fuzzy es esto, calculado sobre la PV ya filtrada y con la ventana
    # medida en segundos, exactamente como en produccion. Antes la simulacion
    # y el motor no coincidian en este paso porque el motor sencillamente no
    # lo tenia.
    defs_online = (definiciones_calculadas if definiciones_calculadas is not None
                   else (DEFINICIONES_CALCULADAS if calcular_vars else {}))
    vars_online = VariablesOnline(defs_online if calcular_vars else {})
    pend_online = PendientesOnline(pendientes_config or {})
    # Mismo objeto que usa el motor en vivo: simulacion y produccion no
    # pueden diferir en las aceleraciones, igual que ya no difieren en las
    # pendientes ni en las variables calculadas.
    acel_online = AceleracionesOnline(aceleraciones_config or {})
    ...
    for _, row in df_iter.iterrows():
        ...
        base_calc = {**{k: v for k, v in row.items() if isinstance(v, (int, float))},
                     **inputs, **setpoints_actuales, "t_s": t_s}
        calculadas, _ = vars_online.actualizar(base_calc, t_s)
        # Una calculada es una variable mas: se fuzzifica y las reglas la
        # nombran, igual que una PV.
        valores = {**inputs, **calculadas}
        pend_out, _ = pend_online.actualizar(valores, t_s)
        acel_out, _ = acel_online.actualizar(valores, t_s)
        ...
        if fired:
            for evento in fired:
                ...
                # El wait solo cuenta si la regla ACTUO. Un SP pegado a su
                # limite absorbe el paso: la regla dispararia, armaria su wait
                # y quedaria callada minutos sin haber movido nada. Mismo
                # criterio que el motor en vivo (ver core/engine/motor.py
                # revertir_waits), para que simulacion y produccion no
                # diverjan justo en el temporizador.
                movio = any(
                    abs(float(v) - float(setpoints_antes.get(k, 0.0))) > 1e-9
                    for k, v in setpoints_actuales.items())
                waits_revertidos = ([] if movio
                                    else motor.revertir_waits(evento, estado_waits))
                ...
                eventos.append({
                    ...
                    "waits_revertidos": " | ".join(waits_revertidos),
                    "movio_sp": bool(movio),
        ...
        rows_resultado.append({
            ...
            **{k: float(v) for k, v in calculadas.items()},
    ...
    if verbose:
        ...
        print(f"Variables calculadas en linea: {len(vars_online.nombres)}")
        print(f"Fuzzy de pendiente: {len(pend_online.nombres)}")
