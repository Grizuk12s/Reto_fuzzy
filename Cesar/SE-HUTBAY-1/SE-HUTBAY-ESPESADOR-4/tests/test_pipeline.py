...
def config_completa(monkeypatch, tmp_path):
    ...
    # Misma leccion A17/A19 para el archivo nuevo: una aceleracion declarada
    # en la planta cambiaria el resultado del suite. `aceleraciones.json` no
    # tiene constante en runner.py — el motor es el unico que lo lee.
    monkeypatch.setattr(st, "ACELERACIONES_JSON",
                        str(tmp_path / "aceleraciones.json"))
...
def test_el_enable_del_dcs_siembra_el_sp_con_la_pv(store, kep, config_completa,
    ...
    motor._run_tick()                       # DENEGADO: el SE no comanda
    # Desde el espejo (2026-09-09) con el permiso denegado SI sale una
    # escritura: el valor del propio DCS. Lo que no puede salir es una ORDEN
    # del experto, y eso es justo lo que distingue el historial de escrituras.
    assert list(motor._historial_escrituras) == [], (
        "sin permiso no puede salir ninguna orden del experto")
    # Antes aca el interno se quedaba en el SP que el DCS tenia al arrancar
    # (42.0). Desde el pegado sigue a la referencia de arranque de la familia
    # —que en esta config es la PV— porque mientras el lazo esta afuera el
    # numero que describe al equipo es el del proceso, no el que el SE
    # recuerda. Ver `_pegar_setpoints_a_planta`.
...
def test_la_siembra_se_repite_en_cada_flanco_del_enable(store, kep, config_completa,
    ...
    n_ordenes = list(motor._historial_escrituras)
    motor._run_tick()
    # Antes esto afirmaba "sin permiso no se mueve ni se siembra": el objetivo
    # interno se quedaba en `lejos`. Desde el pegado, sin permiso el interno
    # SIGUE a la referencia de la familia — que es el punto entero del cambio.
    # Lo que se conserva es lo importante: no sale nada hacia el DCS.
    assert motor._setpoints[sp0] == VALORES[pv0], (
        "sin el lazo, el objetivo interno se pega a la planta")
    assert list(motor._historial_escrituras) == n_ordenes, (
        "sin permiso no puede salir ninguna orden nueva del experto")
...
# ---------------------------------------------------------------
# Aceleraciones en el motor en vivo
# ---------------------------------------------------------------
def _acel_fija(nombre, fuente, estado="ACELERANDO", rate=2.0, aceleracion=1.0):
    """Un `AceleracionesOnline` de mentira que siempre produce lo mismo.

    La matematica del ajuste esta probada en `tests/test_aceleracion.py`. Lo
    que hace falta verificar ACA es el cableado: que la entrada llegue al
    `fuzzy_out` del tick, que reciba sus `NO-<X>` y que una regla la pueda
    nombrar. Fijar el valor evita tener que hacer correr al motor durante toda
    una ventana de segundos reales para producir un numero que ya se probo.
    """
    from core.variables.aceleracion import (ETIQUETAS_ACELERACION,
                                            signo_aceleracion)

    # El signo se DERIVA de la aceleracion fijada en vez de pasarse aparte:
    # asi un doble no puede quedar diciendo ACELERACION_POSITIVA con `2a`
    # negativo, que es una combinacion que el motor no produce nunca.
    signo = signo_aceleracion(aceleracion, 0.01)

    class _Falso:
        nombres = [nombre]

        def variables_fuente(self):
            return {fuente}

        def reset(self):
            pass

        def actualizar(self, valores, t_s):
            if fuente not in (valores or {}):
                return {}, [f"{nombre} (sin dato de {fuente})"]
            return {nombre: {
                "dom": estado, "signo": signo,
                "val": 1.0, "offset": float(aceleracion),
                "pert": {e: (1.0 if e in (estado, signo) else 0.0)
                         for e in ETIQUETAS_ACELERACION},
                "aceleracion": float(aceleracion), "rate": float(rate),
                "variable": fuente, "ventana_s": 5.0, "umbral_estable": 0.01,
                "n_puntos": 5, "es_aceleracion": True,
            }}, []

    return _Falso()


def test_una_aceleracion_llega_al_fuzzy_out_y_dispara_su_regla(store, kep,
                                                               config_completa,
                                                               monkeypatch):
    """El cableado completo: variable -> fuzzy_out -> regla -> setpoint."""
    from config import VARIABLES_PROCESO, SETPOINT_KEYS

    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    acc = f"{pv0}_Acceleration_5s"
    regla = {"id": "R_ACC", "bloque": "estabilidad", "priority": 100.0,
             "weight": 1.0,
             "if": [{"AND": [(acc, "ACELERANDO")]}],
             "then": [{"accion": "SUBIR_SP", "waits": [], "reiniciar_waits": []}]}
    tabla = {sp0: {"belief_axis": [0.0, 1.0],
                   "steps_por_accion": {"SUBIR_SP": [0.0, 5.0]}}}

    motor = _motor_con(monkeypatch, [regla], tabla)
    motor._aceleraciones = _acel_fija(acc, pv0)

    sp_inicial = motor._setpoints[sp0]
    motor._run_tick()

    tz = st._trazas[-1]
    assert any(f["var"] == acc and f["es_aceleracion"] for f in tz["fuzzy"])
    assert motor._setpoints[sp0] > sp_inicial


def test_una_regla_con_NO_ACELERANDO_no_dispara_si_esta_acelerando(store, kep,
                                                                   config_completa,
                                                                   monkeypatch):
    """La negacion no es codigo propio: la genera `expandir_etiquetas_compuestas`.

    Si alguien cambiara las pertenencias a algo no nitido, este test avisa.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS

    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    acc = f"{pv0}_Acceleration_5s"
    regla = {"id": "R_NO_ACC", "bloque": "estabilidad", "priority": 100.0,
             "weight": 1.0,
             "if": [{"AND": [(acc, "NO-ACELERANDO")]}],
             "then": [{"accion": "SUBIR_SP", "waits": [], "reiniciar_waits": []}]}
    tabla = {sp0: {"belief_axis": [0.0, 1.0],
                   "steps_por_accion": {"SUBIR_SP": [0.0, 5.0]}}}

    motor = _motor_con(monkeypatch, [regla], tabla)
    motor._aceleraciones = _acel_fija(acc, pv0, estado="ACELERANDO")
    sp_inicial = motor._setpoints[sp0]
    motor._run_tick()
    assert motor._setpoints[sp0] == sp_inicial

    # Y con la variable DESACELERANDO, la misma regla si dispara.
    motor._aceleraciones = _acel_fija(acc, pv0, estado="DESACELERANDO",
                                      rate=2.0, aceleracion=-1.0)
    motor._run_tick()
    assert motor._setpoints[sp0] > sp_inicial


def test_una_regla_puede_preguntar_por_el_SIGNO_de_la_aceleracion(store, kep,
                                                                  config_completa,
                                                                  monkeypatch):
    """El signo llega al `fuzzy_out` igual que la dinamica, y es INDEPENDIENTE.

    Es lo que el cambio del 2026-09-14 agrega: las dos variables de abajo
    estan las dos ACELERANDO (una sube cada vez mas rapido, la otra BAJA cada
    vez mas rapido), asi que una regla escrita sobre la dinamica no las puede
    distinguir. Sobre el signo, si.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS

    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    acc = f"{pv0}_Acceleration_5s"
    regla = {"id": "R_ACC_NEG", "bloque": "estabilidad", "priority": 100.0,
             "weight": 1.0,
             "if": [{"AND": [(acc, "ACELERACION_NEGATIVA")]}],
             "then": [{"accion": "SUBIR_SP", "waits": [], "reiniciar_waits": []}]}
    tabla = {sp0: {"belief_axis": [0.0, 1.0],
                   "steps_por_accion": {"SUBIR_SP": [0.0, 5.0]}}}

    motor = _motor_con(monkeypatch, [regla], tabla)
    # Sube cada vez mas rapido: ACELERANDO, pero con el signo POSITIVO.
    motor._aceleraciones = _acel_fija(acc, pv0, estado="ACELERANDO",
                                      rate=2.0, aceleracion=1.0)
    sp_inicial = motor._setpoints[sp0]
    motor._run_tick()
    ...
    # Se escapa hacia ABAJO: sigue ACELERANDO, pero el signo cambio.
    motor._aceleraciones = _acel_fija(acc, pv0, estado="ACELERANDO",
                                      rate=-2.0, aceleracion=-1.0)
    motor._run_tick()
    assert motor._setpoints[sp0] > sp_inicial
...
def test_la_traza_publica_el_rate_la_aceleracion_y_el_estado(store, kep,
                                                             config_completa,
                                                             monkeypatch):
    """Sin esto no se puede calibrar `umbral_estable` mirando la pantalla."""
    from config import VARIABLES_PROCESO

    pv0 = VARIABLES_PROCESO[0]
    acc = f"{pv0}_Acceleration_5s"
    motor = _motor_con(monkeypatch, [], {})
    motor._aceleraciones = _acel_fija(acc, pv0, rate=2.0, aceleracion=1.0)
    motor._run_tick()

    fila = st._trazas[-1]["aceleraciones"][0]
    assert fila["var"] == acc and fila["fuente"] == pv0
    assert fila["rate"] == 2.0 and fila["aceleracion"] == 1.0
    assert fila["estado"] == "ACELERANDO"
    # El signo viaja al lado del estado: sin el, la traza no puede explicar
    # por que dispara una regla escrita sobre ACELERACION_NEGATIVA.
    assert fila["signo"] == "ACELERACION_POSITIVA"


def test_una_aceleracion_sin_su_fuente_se_reporta_y_no_tumba_el_tick(store, kep,
                                                                     config_completa,
                                                                     monkeypatch):
    """Degradar por partes: la variable no existe, el tick sigue."""
    motor = _motor_con(monkeypatch, [], {})
    motor._aceleraciones = _acel_fija("acc_huerfana", "variable_que_no_existe")
    motor._run_tick()

    assert motor._last_error is None
    tz = st._trazas[-1]
    assert tz["aceleraciones"] == []
    assert tz["aceleraciones_omitidas"] == [
        "acc_huerfana (sin dato de variable_que_no_existe)"]


def test_las_anotaciones_del_grafico_llevan_la_aceleracion(store, kep,
                                                           config_completa,
                                                           monkeypatch):
    """El Explorador de Series lee la traza, no recalcula por su cuenta.

    Una pantalla que miente sobre el motor es peor que una pantalla sin ese
    dato, asi que la aceleracion que muestra el grafico tiene que salir del
    mismo tick que decidieron las reglas. Este test es el que ata las dos
    puntas: el tick produce la fila y el endpoint la traduce a tag.
    """
    from app import app
    from config import VARIABLES_PROCESO
    ...
    motor._aceleraciones = _acel_fija(acc, pv0, estado="DESACELERANDO",
                                      rate=2.0, aceleracion=-1.0)
    monkeypatch.setattr(st, "_se_engine", motor)
    import web.api.se as se_api
    monkeypatch.setattr(se_api, "_se_engine", motor)
    motor._run_tick()

    d = app.test_client().get("/api/se/grafico/anotaciones").get_json()
    tag_pv = motor._mapeo["tag_to_pv"]
    tag = next(t for t, rol in tag_pv.items() if rol == pv0)
    anot = d["por_tag"][tag]["aceleracion"]
    assert anot is not None
    assert anot["var"] == acc
    assert anot["dom"] == "DESACELERANDO"
    assert anot["rate"] == 2.0 and anot["aceleracion"] == -1.0
    # El signo viaja junto al dominio. Sin el, el tooltip se queda sin nada
    # que decir con la variable a velocidad constante, que es justo cuando
    # `dom` viene vacio.
    assert anot["signo"] == "ACELERACION_NEGATIVA"


def test_sin_aceleracion_declarada_la_anotacion_es_None(store, kep,
                                                        config_completa,
                                                        monkeypatch):
    """No inventa: la ausencia se reporta como ausencia, no como cero."""
    from app import app
    from config import VARIABLES_PROCESO
    ...
    d = app.test_client().get("/api/se/grafico/anotaciones").get_json()
    assert all(v["aceleracion"] is None for v in d["por_tag"].values())


# ---------------------------------------------------------------
# Pegado al SP de planta mientras el SE no tiene el lazo
# ---------------------------------------------------------------
# La siembra por flanco corregia el SP UNA vez, en el instante de la entrega.
# El pegado lo mantiene igual al de planta todo el tiempo que el lazo esta
# afuera, asi que la entrega es bumpless por construccion y no depende de
# acertarle a un evento. Ver "El SP se pega a la planta mientras el SE no
# tiene el lazo" en CLAUDE.md.

def test_el_sp_interno_SIGUE_a_la_planta_mientras_el_enable_esta_denegado(
        store, kep, config_completa, monkeypatch, tmp_path):
    """El caso que la siembra por flanco no cubria: el equipo se mueve.

    Con permiso denegado el operador maneja la bomba a mano. Antes el objetivo
    interno quedaba CONGELADO en el ultimo valor del SE mientras el proceso se
    iba a otro lado; ahora lo sigue tick a tick.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    ...
    # El operador mueve el equipo. Sin pegado, el interno no se enteraba.
    antes = motor._setpoints[sp0]
    VALORES[pv0] += 7.0
    try:
        motor._run_tick()
        assert motor._setpoints[sp0] > antes, (
            "el objetivo interno tiene que seguir al proceso, no quedarse "
            "en el valor de hace un rato")
        tz = st._get_trazas(1)[0]
        assert [e["sp"] for e in tz.get("setpoints_pegados", [])] == [sp0]
        assert list(motor._historial_escrituras) == [], (
            "el espejo no es una orden del experto")

        # Sigue a la PV **filtrada**, asi que converge con el lag del Exp-Q en
        # vez de saltar. Es lo correcto y es lo mismo que ya hacen la siembra y
        # el tracking: todo el pipeline decide sobre la señal filtrada.
        for _ in range(60):
            motor._run_tick()
        assert motor._setpoints[sp0] == pytest.approx(VALORES[pv0], abs=0.5)
    finally:
        VALORES[pv0] -= 7.0


def test_al_conceder_el_enable_SUELTA_y_manda_el_defuzzy(
        store, kep, config_completa, monkeypatch, tmp_path):
    """Soltar no es una accion: es dejar de pegarse.

    Con el permiso concedido la familia sale de las bloqueadas, el pegado no
    corre mas y el primer paso del defuzzy sale del valor con el que el equipo
    venia operando.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    ...
    motor = _motor_con_enable(
        monkeypatch, tmp_path,
        {sp0: {"pv_key": pv0, "rango": 99.0, "habilitado": True}},
        [_regla_simple(pv0)], tabla, kep, permiso_inicial=False)
    ...
    motor._run_tick()
    tz = st._get_trazas(1)[0]
    assert tz.get("setpoints_pegados") is None, "con el lazo dado ya no se pega"
    assert motor._setpoints[sp0] > VALORES[pv0], (
        "el defuzzy tiene que empezar a mandar desde el valor de planta")


def test_el_pegado_NO_pisa_a_una_familia_retenida_por_tracking(
        store, kep, config_completa, monkeypatch, tmp_path):
    """Con tracking el SE SI tiene el lazo: esta esperando al proceso.

    Pegarse ahi borraria el objetivo que el experto esta tratando de alcanzar,
    y el tracking no volveria a liberarse nunca porque el objetivo pasaria a
    ser el propio readback.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    ...
    motor = _motor_con_tracking(
        monkeypatch, tmp_path,
        {sp0: {"pv_key": pv0, "rango": 0.3, "habilitado": True}},
        [_regla_simple(pv0)], tabla)          # handshake apagado: el SE manda

    motor._run_tick()                          # dispara y mueve el SP
    objetivo = motor._setpoints[sp0]
    assert abs(objetivo - VALORES[pv0]) > 0.3, (
        "el SP tiene que quedar fuera del rango del readback para que retenga")

    motor._run_tick()                          # el readback no alcanza: retiene
    tz = st._get_trazas(1)[0]
    assert tz.get("tracking"), "el tracking tiene que estar reteniendo"
    assert tz.get("setpoints_pegados") is None
    assert motor._setpoints[sp0] == pytest.approx(objetivo), (
        "la familia retenida conserva su objetivo; pegarla lo borraria y el "
        "tracking no se liberaria nunca, porque el objetivo pasaria a ser el "
        "propio readback")


def test_sin_referencia_declarada_no_se_pega_nada(store, kep, config_completa,
                                                  monkeypatch, tmp_path):
    """`fuente: ninguno` es una eleccion valida: esa planta no quiere bumpless."""
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    motor = _motor_con_enable(
    ...
    motor._run_tick()
    assert st._get_trazas(1)[0].get("setpoints_pegados") is None
    assert motor._setpoints[sp0] == kep["sp_en_dcs"]


def test_el_handshake_denegado_GANA_sobre_el_tracking_para_el_pegado(
        store, kep, config_completa, monkeypatch, tmp_path):
    """Regresion: el DCS retira el lazo con el tracking ya reteniendo.

    Es el caso mas comun de todos —el tracking retiene justo despues de que el
    SE movio un SP— y estaba roto. `_familias_sin_escritura` arma su dict con
    `setdefault`, asi que el motivo "tracking" entraba primero y tapaba al del
    handshake; decidir el pegado leyendo ese texto daba el resultado al reves y
    el SP interno se quedaba congelado con el lazo afuera.

    Se detecto verificando el bumpless contra la config de planta, no leyendo
    el codigo. Por eso el pegado se decide por CONDICION (`_familias_sin_lazo`)
    y no por el motivo.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    ...
    motor = _motor_con_enable(
        monkeypatch, tmp_path,
        {sp0: {"pv_key": pv0, "rango": 0.3, "habilitado": True}},
        [_regla_simple(pv0)], tabla, kep, permiso_inicial=True)

    motor._run_tick()
    motor._run_tick()
    lejos = motor._setpoints[sp0]
    assert abs(lejos - VALORES[pv0]) > 0.3, "el SP tiene que quedar fuera del rango"
    assert (motor._sp_retenidos or {}).get(sp0), "el tracking tiene que retener"

    kep["handshake"] = False                # el DCS se lleva el lazo
    motor._run_tick()

    tz = st._get_trazas(1)[0]
    assert tz["escritura"].get("escritos") in (None, [], {}), "sin permiso no se escribe"
    assert motor._setpoints[sp0] == VALORES[pv0], (
        "con el lazo afuera manda el pegado, no el objetivo que el tracking "
        "estaba esperando")
    assert [e["sp"] for e in tz.get("setpoints_pegados", [])] == [sp0]


def test_el_pegado_no_produce_un_evento_por_tick_si_nada_se_mueve(
        store, kep, config_completa, monkeypatch, tmp_path):
    """El churn que delato el segundo defecto.

    El alineado (antes y despues de escribir) devolvia el interno a lo ultimo
    escrito y el pegado lo volvia a subir: el numero final era correcto pero se
    rehacia en cada tick, y la traza mostraba un evento de pegado por tick para
    siempre. Con el proceso quieto tiene que haber UNO y despues silencio.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    ...
    motor._run_tick()                        # aca si: adopta la referencia
    assert st._get_trazas(1)[0].get("setpoints_pegados")

    for _ in range(5):
        motor._run_tick()
        assert st._get_trazas(1)[0].get("setpoints_pegados") is None, (
            "con la referencia quieta el pegado no tiene nada que hacer")


def test_el_objetivo_interno_se_graba_como_serie_aunque_no_se_escriba(
        store, kep, config_completa, monkeypatch, tmp_path):
    """Sin esta serie el pegado era invisible en el grafico.

    `_write_setpoints` solo grababa el objetivo de las familias que iba a
    escribir — o sea justo las que no hacen falta para diagnosticar. Con el
    lazo afuera el SE no escribe, el tag de salida queda congelado, y en el
    Explorador de Series parecia que el objetivo interno tampoco se movia.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    serie = st.nombre_serie_interna(sp0)

    with st._tag_history_lock:
        st._tag_history.clear()
    ...
    hist = st._get_tag_history()
    assert serie in hist, "la serie del objetivo interno tiene que existir"
    assert hist[serie][-1]["v"] == motor._setpoints[sp0]
    assert list(motor._historial_escrituras) == [], (
        "y sin que el experto haya comandado nada")


def test_el_catalogo_del_grafico_ofrece_las_series_del_SE(store, monkeypatch, tmp_path):
    """Se ofrecen aunque el motor este detenido: no son tags del KEPserver."""
    from app import app
    from config import SETPOINT_KEYS
    monkeypatch.setattr(st, "_load_tags", lambda: store)
    cat = app.test_client().get("/api/tags/catalogo").get_json()
    series = {c["name"]: c for c in cat if c.get("group") == "se"}
    for sp in SETPOINT_KEYS:
        nombre = st.nombre_serie_interna(sp)
        assert nombre in series, f"falta la serie interna de {sp}"
        assert "objetivo interno" in series[nombre]["pseudonimo"]


# ---------------------------------------------------------------
# La regla del bumpless, dicha como la pide el estandar
# ---------------------------------------------------------------
# FBK en 0 -> el SP del SE se pega al SP del DCS.
# FBK en 1 -> NO se pega.
# Estos dos tests estan escritos con `fuente: tag` apuntando a un SP de
# referencia del DCS, que es como esta la planta, y no con la PV de readback.

def _motor_ref_dcs(monkeypatch, tmp_path, kep, tag_ref, permiso, habilitado=True):
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    return _motor_con_enable(
        monkeypatch, tmp_path,
        {sp0: {"pv_key": pv0, "rango": 0.3, "habilitado": habilitado,
               "arranque": {"fuente": "tag", "tag": tag_ref}}},
        [], {}, kep, permiso_inicial=permiso)


def _tag_ref_del_dcs(kep, valor=72.0):
    """Un SP de referencia del DCS legible por el KEPserver falso.

    El tag NO tiene por que ser una familia del contrato: en la planta es
    justamente un tag de lectura (el setpoint que el operador ve en el HMI).
    Basta con que el prefijo `PLANTA.SP_` lo haga legible por el fixture.

    Antes esto tomaba `SETPOINT_KEYS[-1]` y se saltaba si el contrato traia una
    sola familia. En la corrida completa los tests se saltaban de verdad —otro
    test recarga el contrato antes— asi que no protegian nada y el suite decia
    "pasa" igual. Es la leccion A17 con otra cara: un test no puede depender de
    la config viva.
    """
    tag = "PLANTA.SP_referencia_del_dcs"
    kep["sp_en_dcs_por_tag"][tag] = valor
    return tag


def test_con_el_FBK_en_0_el_SP_del_SE_SIGUE_al_SP_del_DCS(store, kep, config_completa,
                                                          monkeypatch, tmp_path):
    from config import SETPOINT_KEYS
    st._reset_trazas()
    sp0 = SETPOINT_KEYS[0]
    ...
    motor._run_tick()
    assert motor._setpoints[sp0] == 72.0, "se pega al SP del DCS"

    # Y lo SIGUE EN CADA DATO, no una vez. Se mueve la referencia con ruido
    # chico —como oscila un SP real en el HMI— y se exige igualdad EXACTA en
    # todos los ticks.
    #
    # El modo de falla que esto cuida es la banda muerta: `SP_DEADBAND` vale
    # 1e-6, asi que el pegado sigue hasta el ultimo decimal. Si alguien la
    # subiera a, digamos, 0.5 "para no hacer ruido", el SP interno empezaria a
    # quedarse atras de la referencia sin que nada fallara — y la transferencia
    # dejaria de ser bumpless justo cuando el DCS entrega el lazo.
    import math as _math
    peor = 0.0
    valores_vistos = {72.0}
    for i in range(20):
        nuevo = round(72.0 + 1.0 * _math.sin(i / 3.7) + 0.4 * _math.sin(i * 2.3), 4)
        kep["sp_en_dcs_por_tag"][tag_ref] = nuevo
        valores_vistos.add(nuevo)
        motor._run_tick()
        peor = max(peor, abs(motor._setpoints[sp0] - nuevo))
        assert motor._setpoints[sp0] == nuevo, (
            f"tick {i}: con el FBK en 0 el SP interno tiene que valer {nuevo}")
    assert peor == 0.0, "el pegado no puede tener banda muerta"
    # Lo unico que puede salir hacia el DCS es la propia referencia (espejo).
    escritos = [v for w in kep["escrituras"] for t, v in w.items()
                if t == f"PLANTA.SP_{sp0}"]
    assert all(v in valores_vistos for v in escritos), (
        f"solo puede escribirse la referencia, salio {escritos}")
    assert list(motor._historial_escrituras) == [], (
        "y ninguna orden del experto")


def test_con_el_FBK_en_1_el_SP_del_SE_NO_se_pega(store, kep, config_completa,
                                                 monkeypatch, tmp_path):
    """Con el lazo entregado manda el SE: el SP del DCS deja de mandar."""
    from config import SETPOINT_KEYS
    st._reset_trazas()
    sp0 = SETPOINT_KEYS[0]
    tag_ref = _tag_ref_del_dcs(kep, 72.0)
    motor = _motor_ref_dcs(monkeypatch, tmp_path, kep, tag_ref, permiso=True)

    motor._run_tick()
    tomado = motor._setpoints[sp0]

    # El operador mueve su setpoint. El SE ya tiene el lazo: no lo sigue.
    for nuevo in (60.0, 85.0):
        kep["sp_en_dcs_por_tag"][tag_ref] = nuevo
        motor._run_tick()
        assert motor._setpoints[sp0] == tomado, (
            "con el FBK en 1 el SP interno no se pega al del DCS")
        assert st._get_trazas(1)[0].get("setpoints_pegados") is None


def test_el_interruptor_de_tracking_NO_influye_en_el_bumpless(store, kep, config_completa,
                                                              monkeypatch, tmp_path):
    """Son dos cosas distintas sobre la misma familia.

    `habilitado` decide si el tracking BLOQUEA por seguimiento. El bumpless
    decide con que numero se retoma el lazo. `arranque_definido()` no mira
    `habilitado` a proposito: apagar el bloqueo por seguimiento no puede
    dejar a una planta retomando con un salto.
    """
    from config import SETPOINT_KEYS
    sp0 = SETPOINT_KEYS[0]
    resultados = {}
    for habilitado in (True, False):
        st._reset_trazas()
        tag_ref = _tag_ref_del_dcs(kep, 77.0)
        motor = _motor_ref_dcs(monkeypatch, tmp_path, kep, tag_ref,
                               permiso=False, habilitado=habilitado)
        motor._run_tick()
        kep["sp_en_dcs_por_tag"][tag_ref] = 64.0
        motor._run_tick()
        resultados[habilitado] = motor._setpoints[sp0]

    assert resultados[True] == resultados[False] == 64.0, (
        "el bumpless se comporta igual con el tracking habilitado y deshabilitado")


# ---------------------------------------------------------------
# Espejo del SP del DCS con el lazo afuera (SP tracking)
# ---------------------------------------------------------------
# Con el FBK en 0 el SE ESCRIBE en su tag de salida el SP del DCS, para que la
# senal que el DCS ve tampoco tenga salto en la transferencia. Es una excepcion
# deliberada al fail-closed y por eso esta acotada: lo unico que el SE puede
# escribir sin permiso es el numero que acaba de leer de la referencia.

def test_con_el_FBK_en_0_el_SE_ESCRIBE_el_SP_del_DCS_en_su_tag(
        store, kep, config_completa, monkeypatch, tmp_path):
    from config import SETPOINT_KEYS
    st._reset_trazas()
    sp0 = SETPOINT_KEYS[0]
    ...
    esc = st._get_trazas(1)[0]["escritura"]
    assert esc.get("espejo"), "tiene que haber escritura espejo"
    fila = esc["espejo"][0]
    assert fila["sp"] == sp0 and fila["tag"] == tag_sp
    assert fila["valor"] == 72.0 and fila["origen"] == tag_ref
    assert kep["sp_en_dcs_por_tag"][tag_sp] == 72.0, "el DCS recibio el valor"


def test_el_espejo_NUNCA_escribe_un_valor_calculado_por_las_reglas(
        store, kep, config_completa, monkeypatch, tmp_path):
    """La garantia que hace aceptable escribir con el permiso denegado.

    Se monta una regla que dispara y una tabla que la mandaria a mover el SP.
    Con el FBK en 0 lo unico que puede salir hacia el DCS es la referencia.
    """
    from config import VARIABLES_PROCESO, SETPOINT_KEYS
    st._reset_trazas()
    pv0, sp0 = VARIABLES_PROCESO[0], SETPOINT_KEYS[0]
    tag_sp = f"PLANTA.SP_{sp0}"
    tag_ref = _tag_ref_del_dcs(kep, 66.0)
    tabla = {sp0: {"belief_axis": [0.0, 1.0], "steps_por_accion": {"SUBIR_SP": [0.0, 25.0]}}}
    motor = _motor_con_enable(
        monkeypatch, tmp_path,
        {sp0: {"pv_key": pv0, "rango": 99.0, "habilitado": True,
               "arranque": {"fuente": "tag", "tag": tag_ref}}},
    ...
    escritos = [v for w in kep["escrituras"] for t, v in w.items() if t == tag_sp]
    assert escritos, "hubo escritura espejo"
    assert all(v == 66.0 for v in escritos), (
        f"sin permiso solo puede salir la referencia, salio {escritos}")
    assert motor._setpoints[sp0] == 66.0, "y el objetivo interno tampoco acumula"


def test_sin_referencia_legible_el_espejo_no_escribe_nada(store, kep, config_completa,
                                                          monkeypatch, tmp_path):
    """Regla de oro: lo que no se pudo leer no se rellena."""
    from config import SETPOINT_KEYS
    st._reset_trazas()
    sp0 = SETPOINT_KEYS[0]
    tag_ref = _tag_ref_del_dcs(kep, 72.0)
    kep["omitir"].add(tag_ref)                 # el KEPserver no lo entrega
    motor = _motor_ref_dcs(monkeypatch, tmp_path, kep, tag_ref, permiso=False)

    motor._run_tick()
    assert (st._get_trazas(1)[0]["escritura"].get("espejo") or []) == []
    assert kep["escrituras"] == []


def test_con_el_FBK_en_1_no_hay_espejo(store, kep, config_completa,
                                       monkeypatch, tmp_path):
    st._reset_trazas()
    tag_ref = _tag_ref_del_dcs(kep, 72.0)
    motor = _motor_ref_dcs(monkeypatch, tmp_path, kep, tag_ref, permiso=True)
    ...
    motor._run_tick()
    kep["sp_en_dcs_por_tag"][tag_ref] = 55.0
    motor._run_tick()
    assert (st._get_trazas(1)[0]["escritura"].get("espejo") or []) == [], (
        "con el lazo entregado manda el SE, no el SP del DCS")


def test_el_espejo_no_ensucia_el_historial_de_escrituras(store, kep, config_completa,
                                                         monkeypatch, tmp_path):
    """Ese anillo contesta "que le mando el experto"; un espejo es un eco.

    A 1 Hz llenaria las 200 entradas en tres minutos y se llevaria puesta la
    unica auditoria de lo que el SE si decidio.
    """
    from config import SETPOINT_KEYS
    st._reset_trazas()
    tag_ref = _tag_ref_del_dcs(kep, 72.0)
    ...
    motor._run_tick()
    assert st._get_trazas(1)[0]["escritura"].get("espejo"), "hubo espejo"
    assert list(motor._historial_escrituras) == [], (
        "el espejo no entra al historial de escrituras")
