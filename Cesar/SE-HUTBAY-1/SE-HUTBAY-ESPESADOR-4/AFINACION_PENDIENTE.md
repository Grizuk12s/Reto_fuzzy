...
- **`umbral_estable` de las aceleraciones está sin calibrar** (2026-09-08). El default
  0,01 u/s² es una plantilla neutra, igual que los 11 modelos difusos y que `q=0.15` en
  los filtros. La segunda derivada amplifica el ruido, así que un umbral bajo deja el
  estado saltando entre ACELERANDO y DESACELERANDO con el proceso quieto — que es
  exactamente el modo de falla que haría inútil la función. **Método:** abrir
  `/espesador#aceleracion` con el motor corriendo y subir el umbral hasta que una
  variable en régimen se quede en ESTABLE; la columna **Margen** del paso 4 de la traza
  (`|2a| / umbral`) dice cuánto falta. Y decidir con el experto qué variables la
  necesitan y con qué ventana: 5 s detecta un escape, 60 s un cambio de régimen, y se
  pueden declarar las dos sobre la misma variable.
...
```bash
python -m pytest tests/ -q                      # 315 pass, 2 fallan (A17), 7 skip
...
python -m pytest tests/test_aceleracion.py -q   # 43: ajuste cuadratico, estado y API
```
