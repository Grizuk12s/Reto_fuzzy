...
(function (global) {
  ...
  function aceleraciones(before, after) {
    const kd = keyDiff(before, after);
    return join([
      count(Object.keys(after || {}).length, 'aceleracion', 'aceleraciones') + ' en total',
      kd.added.length
        ? count(kd.added.length, 'aceleracion nueva', 'aceleraciones nuevas') + ': ' + fmtList(kd.added) : '',
      kd.removed.length
        ? count(kd.removed.length, 'aceleracion quitada', 'aceleraciones quitadas') + ': ' + fmtList(kd.removed) : '',
    ]);
  }
