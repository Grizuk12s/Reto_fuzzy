...
from core.variables.aceleracion import ETIQUETAS_ACELERACION
...
ACELERACIONES_JSON = os.path.join(_CFG_DIR, "aceleraciones.json")
...
ETIQUETAS_BASE = [
    ...
    "NO-ACELERANDO", "NO-DESACELERANDO", "NO-ESTABLE",
    # Signo de la aceleracion, independiente de la dinamica: ACELERANDO no
    # dice para que lado (una variable que se escapa hacia abajo tambien
    # acelera). Ver ETIQUETAS_SIGNO en core/variables/aceleracion.py.
    "ACELERACION_POSITIVA", "ACELERACION_NEGATIVA", "ACELERACION_NULA",
    "NO-ACELERACION_POSITIVA", "NO-ACELERACION_NEGATIVA", "NO-ACELERACION_NULA",
    "ON", "OFF",
]
            ...
            for etiqueta in (spec.get("labels") or {}):
                for nombre in (str(etiqueta).upper(), f"NO-{str(etiqueta).upper()}"):
                    if nombre not in vistas:
                        vistas.add(nombre)
                        out.append(nombre)
    # Las de aceleracion NO son configurables: son las del estandar (tres
    # dinamicas + tres de signo) y ya estan en ETIQUETAS_BASE. Se recorren
    # igual desde el modulo para que agregar una mas algun dia sea un solo
    # cambio, en un solo lugar.
    for etiqueta in ETIQUETAS_ACELERACION:
        for nombre in (etiqueta, f"NO-{etiqueta}"):
            if nombre not in vistas:
                vistas.add(nombre)
                out.append(nombre)
    return out
...
def etiquetas_por_variable() -> dict:
    ...
    # Una aceleracion tiene etiquetas FIJAS (las del estandar: tres dinamicas
    # y tres de signo, todas a la vez sobre la misma variable), no
    # configurables como las filas de un fuzzy. Se declaran igual POR
    # VARIABLE para que la pagina no ofrezca LOW sobre una aceleracion ni
    # ACELERANDO sobre un nivel: es el defecto A27, que ya se cerro para las
    # pendientes y volveria a abrirse si esto saliera de la union.
    for nombre in nombres_aceleraciones():
        _agregar(nombre, ETIQUETAS_ACELERACION)
...
def aceleraciones_definidas() -> dict:
    """Aceleraciones declaradas en `aceleraciones.json`.

    Se relee en cada llamada, igual que fuzzy.json y pendientes.json: una
    aceleracion recien creada tiene que poder nombrarse en una regla o en un
    estado sin reiniciar el servicio.

    Con `_leer_json` y NO con un `cargar_*` de runner.py: un archivo ausente
    significa "esta planta todavia no declaro ninguna", nunca "usa la
    plantilla de otra planta".
    """
    cfg = _leer_json(ACELERACIONES_JSON, {})
    if not isinstance(cfg, dict):
        return {}
    return {str(k): v for k, v in cfg.items() if isinstance(v, dict)}


def nombres_aceleraciones() -> list[str]:
    return sorted(aceleraciones_definidas())
...
def variables_disponibles() -> list[str]:
    ...
    # Mismo criterio que las pendientes: existen las DECLARADAS, ni una mas.
    # Ofrecer `<pv>_Acceleration_5s` para toda PV seria ofrecer una variable
    # que el motor no produce, y la regla que la nombrara quedaria
    # `no_evaluable` para siempre.
    nombres.extend(v for v in nombres_aceleraciones() if v not in nombres)
...
def roles_en_uso(fuzzy_cfg: dict | None = None) -> dict[str, set[str]]:
    ...
    aceleraciones = _leer_json(ACELERACIONES_JSON, {})
    ...
    # --- Idem para las aceleraciones ---
    # `PU009_Nivel_Acceleration_5s` no se puede calcular sin `PU009_Nivel`,
    # aunque ninguna regla escriba ese nombre. Se suman a `fuentes_pend`, que
    # es lo que ya arrastra a la PV y a la calculada de origen mas abajo.
    acel_en_uso = {n for n in aceleraciones if n in (vars_reglas | vars_perm)}
    fuentes_acel = {str((aceleraciones.get(n) or {}).get("variable") or "")
                    for n in acel_en_uso}
    fuentes_acel.discard("")
    fuentes_pend |= fuentes_acel
    ...
    return {"pv": pv_en_uso, "lim": lim_en_uso, "sp": sp_en_uso,
            ...
            "pendiente": pend_en_uso, "aceleracion": acel_en_uso,
            "otro": set()}
...
# Mensaje FIJO de la alerta de licencia. `AlertCollector.add` deduplica por
# mensaje exacto: con el motivo adentro (el de "reloj retrocedido" trae la hora
# actual) cada llamada abriria una alerta nueva. El motivo va en `detail`.
LICENCIA_ALERTA_MSG = (
    "No hay licencia activa: el SE no escribe setpoints al DCS y el generador "
    "de datos esta detenido. El heartbeat sigue funcionando."
)


def sincronizar_alerta_licencia(lic: dict | None = None) -> dict:
    """Mantiene la alerta de licencia al dia con el estado real.

    Sin licencia valida la alerta queda ACTIVA, y se re-crea aunque el operador
    la marque resuelta: se llama en cada `GET /api/alerts` (la barra de alertas
    consulta cada 5 s) y en cada escritura bloqueada. Con licencia valida se
    cierra la categoria. Devuelve el resultado de `_license_check()`.
    """
    if lic is None:
        lic = _license_check()
    if lic.get("valid"):
        _alerts.resolve_category("licencia")
    else:
        _alerts.add("licencia", LICENCIA_ALERTA_MSG,
                    str(lic.get("reason") or "Sin licencia activa"))
    return lic
...
# Prefijo de las series que NO son tags: las produce el SE y viven en el mismo
# buffer de historial para poder graficarse al lado de las senales de planta.
#
# Existe por un problema concreto de diagnostico: el grafico solo podia dibujar
# TAGS, y el objetivo interno del SE no es un tag. Con el lazo afuera el SE no
# escribe, asi que el tag de salida queda congelado en su ultimo valor — y
# mirando el grafico parecia que el SP interno no hacia nada, cuando en
# realidad estaba siguiendo al SP del DCS tick a tick. Sin esta serie, la unica
# forma de ver el pegado era abrir la traza y leer JSON.
PREFIJO_SERIE_SE = "SE::"


def nombre_serie_interna(sp_key: str) -> str:
    """Nombre de la serie del objetivo interno de una familia de SP."""
    return f"{PREFIJO_SERIE_SE}{sp_key}"
...
class TagGenerator:
    ...
    def _write_tick(self):
        # Sin licencia el generador NO funciona: ni genera, ni graba al
        # historial, ni escribe. Se chequea antes de todo para que el grafico
        # no muestre valores simulados que nunca llegaron al KEPserver.
        lic = _license_check()
        if not lic["valid"]:
            self._last_error = f"Generador bloqueado: {lic['reason']}"
            sincronizar_alerta_licencia(lic)
            return
...
class HeartbeatManager:
    ...
    def _worker(self):
        try:
            while not self._stop_event.is_set():
                try:
                    # El heartbeat NO depende de la licencia, a proposito: es la
                    # senal de vida que el DCS vigila. Sin licencia el SE no
                    # escribe setpoints, pero el proceso sigue vivo y lo dice.
...
def _nueva_traza(tick: int, t_s: float, mapeo: dict) -> dict:
    return {
        ...
        "limites": {}, "derivadas": {},
        "derivadas_omitidas": [], "pendientes_omitidas": [],
        "aceleraciones": [], "aceleraciones_omitidas": [], "tracking": [],
...
def _traza_fuzzy(fuzzy_out: dict, inputs: dict, limites: dict) -> list[dict]:
    ...
    for var, meta in (fuzzy_out or {}).items():
        ...
        filas.append({
            ...
            "es_aceleracion": bool(meta.get("es_aceleracion")),
            "fuente": meta.get("variable"),
            "ventana_s": meta.get("ventana_s"),
            "slope_per_min": meta.get("slope_per_min"),
            "rate": meta.get("rate"),
            "aceleracion": meta.get("aceleracion"),
            "valor": (round(float(inputs[var]), 4) if var in inputs else None),
            "lmin": lim.get("lmin"),
            "lmax": lim.get("lmax"),
            "dom": meta.get("dom"),
            # Solo lo traen las aceleraciones. Va al lado de `dom` y no
            # adentro porque son dos preguntas distintas sobre la misma
            # variable, y `dom` ya lo leen el tooltip del grafico y el CSS.
            "signo": meta.get("signo"),
            "pert": {str(k): round(float(v), 3)
                     for k, v in (meta.get("pert") or {}).items() if float(v) > 0},
        })
    return sorted(filas, key=lambda f: (f["es_pendiente"],
                                       f["es_aceleracion"], f["var"]))
...
class SEEngine:
    ...
    def __init__(self):
        ...
        from core.variables.aceleracion import AceleracionesOnline
        ...
        # Aceleraciones (aceleraciones.json): ajuste cuadratico sobre la
        # ventana, estado nitido ACELERANDO / DESACELERANDO / ESTABLE.
        self._aceleraciones = AceleracionesOnline({})
        ...
        # Espejo del SP del DCS mientras el lazo esta afuera: {sp: (valor, origen)}
        # y el reloj de su ultima escritura por tag. Ver `_escribir_espejo_sp`.
        self._sp_espejo: dict = {}
        self._sp_espejo_t: dict = {}
    ...
    def _init_state(self):
        ...
        # --- Aceleraciones: ajuste cuadratico por ventana ---
        from core.variables.aceleracion import AceleracionesOnline
        cfg_acel = aceleraciones_definidas()
        self._aceleraciones = AceleracionesOnline(cfg_acel)
        self._aceleraciones.reset()
        acel_malas = [n for n, m in cfg_acel.items()
                      if isinstance(m, dict) and m.get("habilitado", True)
                      and n not in self._aceleraciones.nombres]
        if acel_malas:
            advertencias.append("aceleraciones mal formadas (se ignoran): "
                                + ", ".join(sorted(acel_malas)))
        # Una aceleracion sobre una variable que este contrato no tiene NO
        # impide arrancar, pero tampoco se va a producir nunca. Avisarlo es
        # la diferencia entre "todavia no cubrio la ventana" (transitorio) y
        # "esto no existe" (hay que arreglar la config).
        _conocidas = set(variables_disponibles())
        acel_sin_fuente = sorted(
            {v for v in self._aceleraciones.variables_fuente()
             if v not in _conocidas})
        if acel_sin_fuente:
            advertencias.append("aceleraciones sobre variables que no existen: "
                                + ", ".join(acel_sin_fuente))
        ...
        # Si el SE tiene pedido el control al DCS (ENABLE_EXT=True). Lo usa la
        # vigilancia de licencia para soltarlo UNA vez cuando vence.
        self._control_pedido = False
        self._licencia_vigilada_t = None
    ...
    def _read_tags(self) -> tuple[dict, dict, dict]:
        ...
        # Piggyback de las referencias de arranque (`arranque_definido` con
        # `fuente: tag`). Antes se leian a demanda, fuera del lote, y estaba
        # bien mientras la siembra era un disparo en el flanco del ENABLE: eran
        # un puñado de ticks por entrega de lazo. Desde que el SP se PEGA a la
        # planta mientras el SE no tiene el lazo, esa lectura ocurre en CADA
        # tick, y a 20 tick/s seria una lectura extra por tick para siempre.
        # En el lote no cuesta nada, y ademas la referencia queda de la misma
        # pasada que todo lo demas — el motivo por el que el handshake tambien
        # viaja aca.
        for cfg_arr in (getattr(self, "_sp_semilla_cfg", {}) or {}).values():
            ref = (cfg_arr or {}).get("tag")
            if (cfg_arr or {}).get("fuente") == "tag" and ref and ref not in all_tags:
                all_tags.append(ref)
    ...
    def _armar_semilla_por_enable(self) -> bool:
        ...
        **Hoy esto es una red de seguridad, no el mecanismo principal.** Desde
        que el SP interno se PEGA al de planta mientras el SE no tiene el lazo
        (`_pegar_setpoints_a_planta`), al llegar el flanco el interno ya vale la
        referencia y la siembra no encuentra nada que corregir. Sigue existiendo
        para el unico caso que el pegado no cubre: que la referencia no haya
        sido legible ni una sola vez mientras el lazo estuvo afuera. Ahi la
        familia queda fail-closed hasta poder leerla, que es lo correcto.
    ...
    def _sembrar_setpoints(self, valores: dict) -> list[dict]:
        ...
        Red de seguridad del pegado (ver `_pegar_setpoints_a_planta`): con el SP
        interno siguiendo a la planta, al llegar el flanco esto ya no tiene nada
        que corregir y no produce eventos. Actua cuando la referencia no se pudo
        leer ni una vez mientras el lazo estuvo afuera.
    ...
    def _familias_sin_escritura(self) -> dict[str, str]:
        ...
        # El handshake va ANTES que el tracking a proposito. Este dict se arma
        # con `setdefault`, o sea que el primer motivo que entra es el que ve
        # el operador — y si el DCS retiro el lazo, ESE es el motivo, no que el
        # tracking este esperando. Con el orden al reves la traza decia
        # "tracking" mientras el permiso estaba denegado, que manda a mirar el
        # readback cuando el problema es otro.
        if self._handshake_enabled and self._handshake_fbk_tag:
    ...
    def _familias_sin_lazo(self) -> set:
        """Familias cuyo mando NO lo tiene el SE en este tick.

        Es una pregunta distinta de `_familias_sin_escritura()`, que responde
        "esta familia no se escribe" — y eso incluye casos en los que el SE
        **si** manda y solo esta esperando (tracking, o una siembra a medias).

        La distincion importa y no es teorica: se detecto verificando el
        bumpless contra la config de planta. `_familias_sin_escritura` arma su
        dict con `setdefault`, asi que si el tracking ya estaba reteniendo, su
        motivo quedaba registrado y tapaba al del handshake. Decidir el pegado
        mirando ese texto daba el resultado exactamente al reves en el caso mas
        comun: el DCS retiraba el lazo con el tracking activo y el SP interno
        se quedaba congelado en vez de seguir al de planta.

        Por eso esto se calcula por CONDICION y no leyendo motivos:

        * handshake denegado -> el DCS se llevo el lazo, no hay discusion;
        * familia inhibida por configuracion o sin limites -> el SE nunca
          escribe esa familia, asi que su objetivo interno no describe nada.

        El tracking NO entra: ahi el SE manda y esta esperando a que el proceso
        alcance el objetivo que el mismo fijo.
        """
        sin_lazo: set = set()
        if self._handshake_enabled and self._handshake_fbk_tag:
            # Barato: lee de `_last_read`, no abre sesion OPC-UA.
            if self._chequear_handshake_dcs() is not None:
                sin_lazo |= set(self._mapeo.get("sp_to_tag", {}))
        sin_lazo |= set(getattr(self, "_sp_inhibidos", {}) or {})
        sin_lazo |= set(getattr(self, "_sp_sin_limite", {}) or {})
        return sin_lazo

    def _pegar_setpoints_a_planta(self, valores: dict,
                                  sin_lazo: set) -> list[dict]:
        """Mientras el SE no tiene el lazo, su SP interno SIGUE al de planta.

        Es la transferencia bumpless de verdad, y reemplaza al disparo unico en
        el flanco del ENABLE. La diferencia se ve en el caso que rompia: con la
        siembra por flanco, el SP interno quedaba **congelado** en el ultimo
        valor que el SE escribio, a veces durante horas, mientras el operador
        movia el equipo a mano. El numero solo se corregia en el instante de la
        entrega — y si en ese instante la referencia no era legible, la familia
        quedaba fail-closed sin escribir hasta que lo fuera. Pegado, el SP
        interno es en todo momento el de planta, asi que la entrega del lazo es
        bumpless por construccion y no depende de acertarle a un evento.

        Cuando el DCS concede el permiso, la familia sale de `bloqueadas`, esto
        deja de correr y el defuzzy empieza a mandar desde el valor con el que
        el equipo venia operando. **Soltar no es una accion: es dejar de
        pegarse.**

        `sin_lazo` sale de `_familias_sin_lazo()`, que lo decide por CONDICION
        y no leyendo motivos de no-escritura. Las retenidas por tracking no
        estan ahi: en ese caso el SE manda y esta esperando al proceso. Una
        rampa por `rate_sp` tampoco, porque el SE tambien manda.

        El valor sale de la MISMA config que la siembra (`arranque_definido`):
        el SP de referencia del DCS o la PV de readback, segun la familia. No
        hay una segunda fuente de verdad.
        """
        eventos: list[dict] = []
        # `_sp_espejo` es la UNICA fuente de la escritura espejo (ver
        # `_escribir_espejo_sp`). Se rearma entero en cada tick y solo lo llena
        # esta funcion, con el valor recien leido de la referencia. Que sea una
        # variable aparte y no `_setpoints` es la garantia ESTRUCTURAL de que
        # con el permiso denegado no se pueda escribir un valor calculado por
        # las reglas: no hay camino por el que ese numero llegue hasta aca.
        self._sp_espejo = {}
        for sp_key in sorted(sin_lazo or ()):
            cfg = (getattr(self, "_sp_semilla_cfg", {}) or {}).get(sp_key) or {}
            if not cfg or cfg.get("fuente") == "ninguno":
                continue                      # familia sin referencia declarada
            valor, origen = self._valor_de_arranque(sp_key, valores)
            if valor is None:
                continue                      # sin referencia legible: no se inventa
            # Se clipea igual que la siembra: el interno no puede quedar fuera
            # de los limites con los que despues se va a escribir, o el primer
            # paso del defuzzy saldria de un valor que el DCS no acepta.
            lims = self._limites_sp.get(sp_key)
            if lims:
                valor = min(max(valor, float(lims[0])), float(lims[1]))
            # Se guarda SIEMPRE, aunque el interno ya valga eso y no haya
            # evento: el espejo puede necesitar reescribir el tag porque el DCS
            # lo movio por su cuenta, no porque la referencia haya cambiado.
            self._sp_espejo[sp_key] = (float(valor), origen)
            interno = float(self._setpoints.get(sp_key, valor))
            if abs(interno - valor) <= SP_DEADBAND:
                continue
            self._setpoints[sp_key] = float(valor)
            # Si la familia estaba esperando la siembra, esto ya la cumplio:
            # el interno vale la referencia. Dejarla pendiente la mantendria
            # fail-closed sin motivo.
            if sp_key in getattr(self, "_sp_semilla_pendiente", set()):
                self._sp_semilla_pendiente.discard(sp_key)
            eventos.append({
                "sp": sp_key,
                "tag": self._mapeo.get("sp_to_tag", {}).get(sp_key),
                "origen": origen,
                "motivo": (self._sp_sin_escritura or {}).get(sp_key, ""),
                "antes": round(interno, 4),
                "despues": round(float(valor), 4),
                "delta": round(float(valor) - interno, 4),
            })
        return eventos
    ...
    # Cada cuanto el motor revisa la licencia para soltar el control. No hace
    # falta mas fino: vencida la licencia el SE ya no escribe SP (ese chequeo
    # es por escritura); esto solo le avisa al DCS que el lazo no lo tiene nadie.
    LICENCIA_VIGILANCIA_S = 1.0

    def _soltar_control_si_no_hay_licencia(self) -> bool:
        """Sin licencia valida, escribe ENABLE_EXT=False UNA vez.

        Es el mismo camino que `stop()`: el DCS ve que el SE ya no pide el lazo
        y lo retoma con su logica de siempre. Sin esto, una licencia que vence
        con el motor corriendo dejaba ENABLE_EXT=True con un SE que no escribe:
        el DCS creeria que el experto manda y el SP quedaria congelado.

        No se vuelve a pedir el control solo al reactivar la licencia: se
        retoma reiniciando el motor, como siempre. Si la escritura falla se
        reintenta en la proxima revision. Devuelve True si solto en esta llamada.
        """
        if not getattr(self, "_control_pedido", False):
            return False
        ahora = time.monotonic()
        ultimo = getattr(self, "_licencia_vigilada_t", None)
        if ultimo is not None and (ahora - ultimo) < self.LICENCIA_VIGILANCIA_S:
            return False
        self._licencia_vigilada_t = ahora
        lic = _license_check()
        if lic.get("valid"):
            return False
        sincronizar_alerta_licencia(lic)
        if self._pedir_control_dcs(False):
            self._control_pedido = False
            _alerts.add("licencia",
                        "Sin licencia: el SE solto el control externo "
                        "(ENABLE_EXT=False). Para retomarlo, activar una licencia "
    ...
    def _write_setpoints(self) -> dict:
        ...
        # El objetivo interno va a su PROPIA serie, y se graba SIEMPRE —
        # inhibida la familia o no. La linea de arriba solo graba las que se
        # van a escribir, o sea justo las que no hacen falta para diagnosticar:
        # cuando el SE no tiene el lazo no escribe nada, el tag de salida queda
        # congelado, y en el grafico parecia que el objetivo interno tampoco se
        # movia. Con esta serie se ve al SP interno pegado al del DCS mientras
        # el FBK esta denegado, y separandose cuando el SE toma el mando.
        _record_tag_values({
            nombre_serie_interna(sp_key): float(valor)
            for sp_key, valor in (self._setpoints or {}).items()
    ...
    # Cada cuanto, como mucho, se reescribe el espejo de un tag. El SP del DCS
    # oscila de continuo y el lazo corre en ciclo libre: sin este piso serian
    # ~20 escrituras por segundo por familia, para siempre, sobre un DCS que ni
    # siquiera esta autorizando al SE. A 1 Hz el tag queda igualado a la vista
    # y en la entrega del lazo manda la escritura normal, que es exacta.
    ESPEJO_MIN_INTERVALO_S = 1.0

    def _escribir_espejo_sp(self) -> list[dict]:
        """Con el lazo AFUERA, escribe en el tag del SE el SP del DCS.

        Es el patron clasico de *SP tracking* de un DCS: mientras el control
        externo no esta autorizado, su entrada de setpoint sigue a la del lazo
        activo, de modo que la transferencia no produce ningun salto — ni
        siquiera transitorio en la senal que el DCS ve.

        **Esto escribe al DCS con el handshake DENEGADO**, que es una excepcion
        deliberada al fail-closed, y por eso esta acotada por construccion:

        * El valor sale de `_sp_espejo`, que llena UNICAMENTE
          `_pegar_setpoints_a_planta` con el numero recien leido de la
          referencia. Un valor calculado por las reglas no tiene camino hasta
          aca: lo peor que el SE puede hacer sin permiso es devolverle al DCS
          su propio numero.
        * Sin referencia legible no se escribe nada. La regla de oro sigue en
          pie: lo que no se pudo leer no se rellena.
        * Solo actua sobre familias sin lazo. Las que el SE si comanda pasan
          por `_write_setpoints`, con su rate limit y su tracking.
        * Se respeta la licencia, igual que cualquier escritura.

        NO entra al historial de escrituras a proposito. Ese anillo contesta
        "que le mando el experto a la planta"; un espejo no es una orden, es un
        eco. A 1 Hz llenaria las 200 entradas en tres minutos y se llevaria
        puesta la unica auditoria de lo que el SE si decidio. Se ve en la traza,
        que es donde corresponde.
        """
        espejo = getattr(self, "_sp_espejo", None)
        if not espejo:
            return []

        pendientes: dict[str, float] = {}
        origenes: dict[str, str] = {}
        for sp_key, (valor, origen) in espejo.items():
            tag = self._mapeo.get("sp_to_tag", {}).get(sp_key)
            if not tag:
                continue
            ultimo = self._sp_espejo_t.get(tag)
            if ultimo is not None and (self._t_s - ultimo) < self.ESPEJO_MIN_INTERVALO_S:
                continue
            escrito = self._sp_escritos.get(tag)
            if escrito is not None and abs(float(valor) - float(escrito)) <= SP_DEADBAND:
                continue
            pendientes[tag] = float(valor)
            origenes[tag] = origen
        ...
        try:
            res = _kep.write_float_batch(pendientes) or {}
        except Exception as e:                                  # noqa: BLE001
            _alerts.add("kep", f"Espejo de SP: {e}", traceback.format_exc())
            return []

        escritos = list(res.get("escritos", list(pendientes)))
        fallidos = dict(res.get("fallidos", {}))
        tag_a_sp = {t: k for k, t in self._mapeo.get("sp_to_tag", {}).items()}
        detalle: list[dict] = []
        for tag in sorted(set(escritos) | set(fallidos)):
            ok = tag in escritos
            if ok:
                self._sp_escritos[tag] = float(pendientes[tag])
                self._sp_espejo_t[tag] = self._t_s
            detalle.append({
                "sp": tag_a_sp.get(tag, ""),
                "tag": tag,
                "valor": round(float(pendientes[tag]), 4),
                "origen": origenes.get(tag, ""),
                "ok": ok,
                "error": None if ok else fallidos.get(tag),
            })
        return detalle
    ...
    def _run_tick(self):
        ...
        # Licencia vencida con el motor corriendo: soltar el lazo al DCS.
        self._soltar_control_si_no_hay_licencia()
        ...
        # --- Aceleraciones: ajuste cuadratico sobre la ventana ---
        # Se calculan sobre `valores` (PV filtradas + calculadas), igual que
        # las pendientes. Sobre la senal cruda no tendrian sentido: la
        # segunda derivada amplifica el ruido que el filtro Exp-Q ya decidio
        # ignorar.
        acel_out, acel_omitidas = self._aceleraciones.actualizar(valores, self._t_s)
        tz["aceleraciones_omitidas"] = acel_omitidas
        tz["aceleraciones"] = [
            {"var": nombre,
             "fuente": meta["variable"],
             "ventana_s": meta["ventana_s"],
             "rate": round(float(meta["rate"]), 6),
             "aceleracion": round(float(meta["aceleracion"]), 6),
             "umbral_estable": meta["umbral_estable"],
             "n_puntos": meta["n_puntos"],
             # `estado` puede venir vacio: es el caso de velocidad constante,
             # que no tiene etiqueta dinamica y se describe con el signo.
             "estado": meta["dom"],
             "signo": meta.get("signo")}
            for nombre, meta in sorted(acel_out.items())]
        ...
        fuzzy_out = _evaluar_estado_fuzzy(
            ...
            pend_modelos={},
            # Las dos entran ANTES de expandir las etiquetas compuestas: si
            # no, se quedarian sin sus NO-<X> y una regla que use
            # NO-ACELERANDO no se podria evaluar.
            pend_extra={**pend_out, **acel_out},
        )
        ...
        sin_lazo = self._familias_sin_lazo()
        # Las familias SIN LAZO quedan fuera del alineado: su objetivo lo fija
        # el pegado de mas abajo. Alinearlas primero y pegarlas despues da el
        # mismo numero, pero con churn: cada tick bajaba el interno a lo ultimo
        # escrito y el pegado lo volvia a subir, asi que la traza mostraba un
        # evento de pegado por tick aunque nada se moviera.
        alineados = self._alinear_setpoints_con_escrito(
            set(self._sp_sin_escritura) - sin_lazo)
        if alineados:
            tz["setpoints_alineados"] = alineados

        # Fase 2b: y mientras el SE no tiene el lazo, el objetivo interno se
        # PEGA al SP de planta. Va DESPUES de alinear a proposito: alinear
        # adopta lo ultimo que el SE escribio, que es una referencia del SE;
        # pegar adopta lo que el equipo esta haciendo de verdad, que es la que
        # vale mientras manda otro. Cuando el DCS concede el permiso esto deja
        # de correr solo y el defuzzy arranca desde ahi: eso es soltar.
        pegados = self._pegar_setpoints_a_planta(valores, sin_lazo)
        if pegados:
            tz["setpoints_pegados"] = pegados
        ...
        # Espejo: con el lazo afuera, el tag del SE sigue al SP del DCS para
        # que la transferencia no produzca salto ni siquiera en la senal que el
        # DCS ve. Va DESPUES de la escritura normal — son conjuntos disjuntos,
        # pero el orden deja claro cual manda si algun dia dejaran de serlo.
        espejo = self._escribir_espejo_sp()
        if espejo:
            tz["escritura"]["espejo"] = espejo
        # Las familias SIN LAZO se excluyen: su objetivo interno lo fija el
        # pegado a la planta, no lo ultimo que el SE escribio. Sin esta resta
        # el pegado se deshacia al final de cada tick y volvia a hacerse al
        # principio del siguiente — el SP interno nunca se movia y la traza
        # mostraba un evento de pegado por tick, para siempre. Detectado
        # verificando el bumpless contra la config de planta, no leyendo.
        post_alineados = self._alinear_setpoints_con_escrito(
            {d["sp"] for d in (tz["escritura"].get("inhibidos") or [])} - sin_lazo)
        if post_alineados:
    ...
    def start(self, piso_s: float | None = None,
        ...
        # Sin licencia NO se pide: el SE no va a escribir, y pedir el lazo
        # haria creer al DCS que el experto manda.
        lic = _license_check()
        if lic.get("valid"):
            self._control_pedido = self._pedir_control_dcs(True)
        else:
            self._control_pedido = False
            sincronizar_alerta_licencia(lic)
        return {"ok": True, "error": None}
