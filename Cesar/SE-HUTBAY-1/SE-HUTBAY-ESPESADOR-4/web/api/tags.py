...
from web.state import (
    ...
    _record_tag_values, _get_tag_history, nombre_serie_interna, _leer_json,
    _tag_generator,
    _heartbeat,
    _se_engine,
    _license_check,
...
def api_tags_catalogo():
    ...
    # Series del propio SE (grupo "se"): el objetivo interno de cada familia
    # de SP. No son tags — no existen en el KEPserver — pero viven en el mismo
    # buffer de historial y son lo unico que permite VER en el grafico que el
    # SP interno sigue al del DCS mientras el lazo esta afuera. Sin esto habia
    # que leer la traza en JSON para saberlo.
    #
    # Se ofrecen aunque todavia no tengan muestras: el motor puede estar
    # detenido y el operador querer dejarlas seleccionadas de antemano.
    import config as _cfg
    descripciones = (_leer_json(_cfg.CONTRATO_JSON, {}) or {}).get("descripciones") or {}
    for sp_key in list(getattr(_cfg, "SETPOINT_KEYS", []) or []):
        out.append({
            "name":       nombre_serie_interna(sp_key),
            "group":      "se",
            "pseudonimo": f"{descripciones.get(sp_key) or sp_key} \u2014 objetivo interno del SE",
            "unidad_ing": "",
            "enabled":    True,
        })
    return jsonify(out)
