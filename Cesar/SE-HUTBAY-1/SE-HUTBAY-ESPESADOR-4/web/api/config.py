...
from web.state import (
    ...
    aceleraciones_definidas, nombres_aceleraciones, ACELERACIONES_JSON,
...
def _variables_huerfanas_por_guardar(actual: dict, nuevo: dict) -> list[str]:
    """Variables que alguna regla nombra y el guardado dejaria sin definir.
    ...
    Es generico a proposito: lo usan las pendientes y las aceleraciones, que
    tienen el mismo problema. Antes era `_pendientes_huerfanas_por_guardar` y
    no miraba nada especifico de una pendiente.
    """
...
# Alias historico: el nombre viejo sigue siendo el que se lee en el bloque de
# pendientes, y renombrar los call sites no agregaba nada.
_pendientes_huerfanas_por_guardar = _variables_huerfanas_por_guardar
...
# ============================================================
# Aceleraciones — aceleraciones.json
# ============================================================
# Una o varias por variable, cada una con su ventana en segundos. A diferencia
# de una pendiente, aca NO hay eje ni filas que calibrar: las seis etiquetas
# son fijas (tres dinamicas y tres de signo) y lo unico configurable es donde
# termina la banda muerta. Ver "La aceleracion es un estado nitido, no un
# fuzzy" en CLAUDE.md.

from core.variables.aceleracion import (
    ETIQUETAS_ACELERACION,
    ETIQUETAS_DINAMICAS as ACEL_ETIQUETAS_DINAMICAS,
    ETIQUETAS_SIGNO as ACEL_ETIQUETAS_SIGNO,
    MIN_PUNTOS as ACEL_MIN_PUNTOS,
    UMBRAL_ESTABLE_DEFAULT as ACEL_UMBRAL_DEFAULT,
    VENTANA_S_DEFAULT as ACEL_VENTANA_DEFAULT,
    nombre_sugerido as acel_nombre_sugerido,
)

# Tope de ventana. No es una restriccion tecnica: es que una aceleracion de
# media hora ya no dice nada sobre lo que la variable esta haciendo ahora, y
# quien quiera esa pregunta tiene las pendientes, que existen justamente para
# mirar derivas lentas.
ACEL_VENTANA_MAX_S = 3600.0


def _defaults_aceleraciones() -> dict:
    """Vacio. Como todo lo demas desde el 2026-09-01: no hay plantilla que sembrar."""
    return {}


def _save_aceleraciones(cfg: dict) -> None:
    escribir_json_atomico(ACELERACIONES_JSON, cfg)


def _load_aceleraciones() -> dict:
    if not os.path.exists(ACELERACIONES_JSON):
        cfg = _defaults_aceleraciones()
        _save_aceleraciones(cfg)
        return cfg
    try:
        with open(ACELERACIONES_JSON, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, ValueError):
        return _defaults_aceleraciones()
    return data if isinstance(data, dict) else _defaults_aceleraciones()


def _fuentes_aceleracion_disponibles() -> list[dict]:
    """Sobre que se puede calcular una aceleracion: PV y calculadas.

    Las mismas que una pendiente: se evalua sobre `valores`, que en el tick
    trae las PV filtradas y las calculadas de ese tick.
    """
    return entradas_fuzzificables()


def _validar_aceleracion_spec(nombre: str, cfg) -> tuple[dict | None, str | None]:
    if not isinstance(cfg, dict):
        return None, f"'{nombre}': debe ser un objeto."

    variable = str(cfg.get("variable") or "").strip()
    if not variable:
        return None, (f"'{nombre}': falta 'variable' (la senal a la que se le "
                      "mide la aceleracion).")
    fuentes = {i["identificador"] for i in _fuentes_aceleracion_disponibles()}
    if fuentes and variable not in fuentes:
        return None, (f"'{nombre}': '{variable}' no es una PV ni una variable "
                      f"calculada. Validas: {sorted(fuentes)}.")

    try:
        ventana_s = float(cfg.get("ventana_s", ACEL_VENTANA_DEFAULT))
    except (TypeError, ValueError):
        return None, f"'{nombre}': 'ventana_s' debe ser numerico."
    if ventana_s <= 0.0:
        return None, f"'{nombre}': 'ventana_s' debe ser > 0."
    if ventana_s > ACEL_VENTANA_MAX_S:
        return None, (f"'{nombre}': 'ventana_s' no puede pasar de "
                      f"{ACEL_VENTANA_MAX_S:.0f} s. Para una deriva lenta usa "
                      "una pendiente, que es la que mide tendencia.")

    try:
        umbral = float(cfg.get("umbral_estable", ACEL_UMBRAL_DEFAULT))
    except (TypeError, ValueError):
        return None, f"'{nombre}': 'umbral_estable' debe ser numerico."
    if umbral < 0.0:
        return None, (f"'{nombre}': 'umbral_estable' no puede ser negativo "
                      "(es una magnitud: se compara contra |2a|).")

    # Opcional. Nace en null y ahi se DERIVA de umbral_estable * ventana_s,
    # que es el rate que esa aceleracion produce en una ventana. Igual que el
    # respaldo numerico de los limites: un valor que vale solo por existir es
    # lo que vuelve invisible al que decide.
    umbral_rate = cfg.get("umbral_rate", None)
    if umbral_rate is not None and str(umbral_rate).strip() != "":
        try:
            umbral_rate = float(umbral_rate)
        except (TypeError, ValueError):
            return None, f"'{nombre}': 'umbral_rate' debe ser numerico o vacio."
        if umbral_rate < 0.0:
            return None, f"'{nombre}': 'umbral_rate' no puede ser negativo."
    else:
        umbral_rate = None

    try:
        min_puntos = int(cfg.get("min_puntos", ACEL_MIN_PUNTOS))
    except (TypeError, ValueError):
        return None, f"'{nombre}': 'min_puntos' debe ser entero."
    if min_puntos < ACEL_MIN_PUNTOS:
        return None, (f"'{nombre}': 'min_puntos' no puede bajar de "
                      f"{ACEL_MIN_PUNTOS}: una cuadratica tiene tres "
                      "incognitas y con menos puntos el ajuste no queda "
                      "determinado.")
    ...
    return {"variable": variable,
            ...
            "descripcion": str(cfg.get("descripcion", "") or "")}, None


def _normalizar_aceleraciones_payload(data) -> tuple[dict | None, str | None]:
    if not isinstance(data, dict):
        return None, ("El payload debe ser un objeto "
                      "{ <nombre>: {variable, ventana_s, umbral_estable} }.")
    # Una aceleracion es una variable mas para las reglas: su nombre no puede
    # chocar con una PV, una cruda, una calculada, un setpoint ni una
    # pendiente.
    ocupados = (set(VARIABLES_PROCESO) | set(SETPOINT_KEYS)
                | set(nombres_calculadas()) | set(nombres_pendientes()) | {"t_s"})
    out = {}
    for nombre, cfg in data.items():
        nombre_s = str(nombre).strip()
        if not VAR_NOMBRE_RE.match(nombre_s):
            return None, (f"'{nombre_s}': nombre invalido. Usa letras, numeros y "
                          "guion bajo, empezando por letra.")
        if nombre_s in ocupados:
            return None, (f"'{nombre_s}' ya existe como PV, setpoint, variable "
                          "calculada o pendiente. Elegi otro nombre.")
        spec, error = _validar_aceleracion_spec(nombre_s, cfg)
        if error is not None:
            return None, error
        out[nombre_s] = spec
    return out, None
...
@bp_config.route("/api/aceleraciones", methods=["GET"])
def api_get_aceleraciones():
    """Aceleraciones declaradas + catalogo de fuentes posibles."""
    actual = _load_aceleraciones()
    fuentes = _fuentes_aceleracion_disponibles()
    return jsonify({
        "actual": actual,
        "fuentes": fuentes,
        "etiquetas": list(ETIQUETAS_ACELERACION),
        # Las dos familias por separado, ademas de la lista completa. La
        # pagina pinta una pastilla por familia y necesita saber cual es
        # cual; derivarlo en el navegador (por prefijo del nombre, por
        # ejemplo) seria una segunda definicion del catalogo que se
        # desincroniza sola en cuanto se agregue una etiqueta.
        "etiquetas_dinamicas": list(ACEL_ETIQUETAS_DINAMICAS),
        "etiquetas_signo": list(ACEL_ETIQUETAS_SIGNO),
        "plantilla": {"ventana_s": ACEL_VENTANA_DEFAULT,
                      "umbral_estable": ACEL_UMBRAL_DEFAULT,
                      "umbral_rate": None,
                      "min_puntos": ACEL_MIN_PUNTOS,
                      "habilitado": True},
        "limites": {"ventana_max_s": ACEL_VENTANA_MAX_S,
                    "min_puntos_min": ACEL_MIN_PUNTOS},
        # Varias por variable a proposito: 5 s para un escape y 60 s para un
        # cambio de regimen son dos preguntas distintas.
        "por_variable": {i["identificador"]: sorted(
                             n for n, c in actual.items()
                             if (c or {}).get("variable") == i["identificador"])
                         for i in fuentes},
        # Para que la pagina pueda avisar ANTES de guardar que borrar o
        # renombrar dejaria una regla muda.
        "uso_en_reglas": {f"{var}|{et}": ids
                          for (var, et), ids in etiquetas_usadas_por_reglas().items()},
    })
...
def api_put_aceleraciones():
    body = request.get_json(force=True) or {}
    forzar = bool(body.pop("__forzar__", False))
    norm, error = _normalizar_aceleraciones_payload(body)
    if error is not None:
        return jsonify({"error": error}), 400
    if not forzar:
        problemas = _variables_huerfanas_por_guardar(_load_aceleraciones(), norm)
        if problemas:
            return jsonify({
...
@bp_config.route("/api/aceleraciones/<nombre>", methods=["POST"])
def api_crear_aceleracion(nombre: str):
    """Crea una aceleracion con la plantilla neutra sobre la variable indicada."""
    body = request.get_json(force=True) or {}
    cfg = _load_aceleraciones()
    nombre_s = str(nombre).strip()
    if nombre_s in cfg:
        return jsonify({"error": f"'{nombre_s}' ya existe."}), 409
    nuevo = {
        "variable": str(body.get("variable") or "").strip(),
        "ventana_s": body.get("ventana_s", ACEL_VENTANA_DEFAULT),
        "umbral_estable": body.get("umbral_estable", ACEL_UMBRAL_DEFAULT),
        "umbral_rate": body.get("umbral_rate", None),
        "min_puntos": body.get("min_puntos", ACEL_MIN_PUNTOS),
        "habilitado": body.get("habilitado", True),
        "descripcion": body.get("descripcion", ""),
    }
    candidato = dict(cfg)
    candidato[nombre_s] = nuevo
    norm, error = _normalizar_aceleraciones_payload(candidato)
    if error is not None:
        return jsonify({"error": error}), 400
    _save_aceleraciones(norm)
    return jsonify({"ok": True, "nombre": nombre_s, "actual": norm,
                    "aviso": "Creada con un umbral de ESTABLE neutro. "
                             "Calibralo mirando la traza antes de usarla en "
                             "una regla: la segunda derivada amplifica el "
                             "ruido."}), 201


@bp_config.route("/api/aceleraciones/sugerir-nombre", methods=["GET"])
def api_sugerir_nombre_aceleracion():
    """`<variable>_Acceleration_<n>s`, el del estandar, ya libre de colisiones.

    El nombre queda CONGELADO al crear: derivarlo de la ventana haria que
    cambiar de 5 s a 10 s renombrara la variable y dejara mudas las reglas que
    la nombran. Esto es solo la propuesta inicial.
    """
    variable = str(request.args.get("variable", "")).strip()
    try:
        ventana_s = float(request.args.get("ventana_s", ACEL_VENTANA_DEFAULT))
    except (TypeError, ValueError):
        ventana_s = ACEL_VENTANA_DEFAULT
    if not variable:
        return jsonify({"error": "Falta 'variable'."}), 400

    base = acel_nombre_sugerido(variable, ventana_s)
    ocupados = (set(_load_aceleraciones()) | set(VARIABLES_PROCESO)
                | set(SETPOINT_KEYS) | set(nombres_calculadas())
                | set(nombres_pendientes()))
    nombre, n = base, 2
    while nombre in ocupados:
        nombre, n = f"{base}_{n}", n + 1
    return jsonify({"nombre": nombre, "base": base})


@bp_config.route("/api/aceleraciones/<nombre>", methods=["DELETE"])
def api_borrar_aceleracion(nombre: str):
    cfg = _load_aceleraciones()
    nombre_s = str(nombre).strip()
    if nombre_s not in cfg:
        return jsonify({"error": f"'{nombre_s}' no existe."}), 404
    ...
    restante = {k: v for k, v in cfg.items() if k != nombre_s}
    if not forzar:
        problemas = _variables_huerfanas_por_guardar(cfg, restante)
        if problemas:
            return jsonify({"error": ("No se borro: " + " | ".join(problemas)),
                            "huerfanas": problemas}), 409
...
@bp_config.route("/api/aceleraciones/vaciar", methods=["POST"])
def api_vaciar_aceleraciones():
    """Deja `aceleraciones.json` en {}.

    Vaciar, no "restaurar default": desde el 2026-09-01 no hay ninguna
    plantilla de otra planta que sembrar (A28).
    """
    _save_aceleraciones({})
    return jsonify({"ok": True, "actual": {}})
