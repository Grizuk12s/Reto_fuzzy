...
def api_alerts():
    # Sin licencia la alerta tiene que estar SIEMPRE, aunque el motor este
    # parado o el operador la haya marcado resuelta: se re-sincroniza en cada
    # consulta (la barra de alertas pregunta cada 5 s).
    try:
        sincronizar_alerta_licencia()
...
def api_grafico_anotaciones():
    """Dominio, pendiente y aceleracion POR TAG, de la ultima traza del pipeline.
    ...
    La pendiente y la aceleracion de una variable se buscan entre las filas
    marcadas como tales cuya `fuente` es esa variable. Sin motor corriendo no
    hay traza: se devuelve running=false y el grafico lo dice en vez de
    inventar.

    Si hay VARIAS pendientes (o aceleraciones) sobre la misma variable se
    reporta la primera. Es deliberado: el tooltip tiene lugar para un numero,
    y elegir en silencio "la mas corta" o "la mas larga" seria una regla
...
def _ejecutar_simulacion(params: dict) -> dict:
    ...
    from runner import cargar_pendientes_json
    ...
    resultados = correr_prueba_general(
        ...
        # Vacio a proposito: PEND_MODELOS era el dict del espesador viejo. Las
        # pendientes de verdad salen de pendientes.json, abajo.
        pend_modelos={},
        pendientes_config=cargar_pendientes_json(),
