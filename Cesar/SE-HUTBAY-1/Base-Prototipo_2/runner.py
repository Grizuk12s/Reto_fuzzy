# -*- coding: utf-8 -*-
"""Runner del sistema experto Espesador (v5).
...
Pipeline por fila:
  ...
  6. Evaluar tracking PV-SP de cada variable manipulada
  7. Motor de reglas con jerarquia de bloques + waits + tracking
  8. Aplicar acciones disparadas sobre SPs (defuzzy Sugeno por tabla)
...
from .tracking import evaluar_tracking, normalizar_config_tracking
...
def extraer_pv_tracking_desde_row(
    row: pd.Series,
    tracking_config: dict,
    columnas_entrada: dict | None = None,
) -> dict:
    """Extrae los PV/readbacks declarados por la configuracion de tracking."""
    columnas_entrada = COLUMNAS_ENTRADA if columnas_entrada is None else columnas_entrada
    pvs = {}
    for spec in tracking_config.values():
        pv_key = str(spec["pv_key"])
        col = _resolver_col(pv_key, columnas_entrada)
        if col not in row.index:
            if not bool(spec.get("habilitado", True)):
                continue
            raise KeyError(
                f"Falta la columna {col!r} (rol {pv_key!r}) requerida por tracking. "
                "Agreguela al DataFrame, cambie tracking_config o use usar_tracking=False."
            )
        pvs[pv_key] = float(row[col])
    return pvs
...
def correr_prueba_general(
    ...
    usar_tracking : bool (default True)
        Activa el bloqueo por seguimiento entre el PV y el SP objetivo.
        Use False para reproducir el comportamiento anterior con historicos
        que no tengan columnas de PV/readback.
    tracking_config : dict | None
        Configuracion parcial por familia. Cada entrada admite ``pv_key``,
        ``rango`` y ``habilitado``. El rango es absoluto y usa las mismas
    ...
    tracking_config_norm = normalizar_config_tracking(tracking_config)
    ...
    for _, row in df_iter.iterrows():
        ...
        if usar_tracking:
            pv_tracking = extraer_pv_tracking_desde_row(
                row,
                tracking_config=tracking_config_norm,
                columnas_entrada=columnas_entrada,
            )
            estado_tracking = evaluar_tracking(
                pv_actuales=pv_tracking,
                setpoints=setpoints_actuales,
                config=tracking_config_norm,
            )
        else:
            estado_tracking = {
                familia: {
                    ...
                    "pv_key": spec["pv_key"],
                    "pv": None,
                    "sp": float(setpoints_actuales[familia]),
                    "error": None,
                    "error_abs": None,
                ...
                for familia, spec in tracking_config_norm.items()
            }
        ...
        for bloqueo in motor_out["bloqueos_tracking"]:
            bloqueos_tracking.append(
                {
                    "t_min": t_s / 60.0,
                ...
                eventos.append(
                    {
                        ...
                        "acciones_bloqueadas_tracking": " | ".join(
                            evento.get("acciones_bloqueadas_tracking", [])
                        ),
                        "sp_afectados": " | ".join(
        ...
        rows_resultado.append(
            {
                ...
                **{f"perm_{k}": bool(v) for k, v in estados_permisivos.items()},
                ...
                "n_acciones_bloqueadas_tracking": len(motor_out["bloqueos_tracking"]),
                "acciones_bloqueadas_tracking": " | ".join(
                    b["accion"] for b in motor_out["bloqueos_tracking"]
                ),
                **{
                    f"tracking_{familia}_bloqueada": bool(estado["bloqueada"])
                    for familia, estado in estado_tracking.items()
                },
                **{
                    f"tracking_{familia}_pv": estado["pv"]
                    for familia, estado in estado_tracking.items()
                },
                **{
                    f"tracking_{familia}_sp": float(estado["sp"])
                    for familia, estado in estado_tracking.items()
                },
                **{
                    f"tracking_{familia}_error": estado["error"]
                    for familia, estado in estado_tracking.items()
                },
                **{
                    f"tracking_{familia}_rango": float(estado["rango"])
                    for familia, estado in estado_tracking.items()
                },
            }
        )
    ...
    df_bloqueos_tracking = pd.DataFrame(bloqueos_tracking)

    if verbose:
        print("=" * 110)
        print("PRUEBA GENERAL DEL SISTEMA EXPERTO ESPESADOR  [v5 -- tracking PV-SP]")
        print("=" * 110)
        if calcular_vars:
        ...
        print(
            "Tracking PV-SP: "
            + ("ACTIVO" if usar_tracking else "DESACTIVADO")
            + f" | Acciones bloqueadas: {len(df_bloqueos_tracking):,}"
        )
    ...
    return {
        ...
        "eventos": df_eventos,
        "bloqueos_tracking": df_bloqueos_tracking,
        "tracking_config": tracking_config_norm,
        "filtro_exp_q": filtro,
    }
